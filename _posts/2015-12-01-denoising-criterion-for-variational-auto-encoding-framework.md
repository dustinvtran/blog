...
Auto-encoders are used to learn representations of observed data $$x$$. They do so by minimizing the reconstruction error of a decoder $$p(x\mid z)$$, given a characterization of hidden variables $$z$$ from an encoder $$q(z\mid x)$$. Along with a penalty $$P_\alpha$$ for regularization, this can be written as maximizing the objective

$$
\mathcal{L}_{\text{ae}} = \mathbb{E}_{q(z\mid x)}[\log p(x\mid z)] - P_\alpha
$$
...
$$
\mathcal{L}_{\text{vae}} = \mathbb{E}_{q(z\mid x)}[\log p(x\mid z)] - \mathrm{KL}(q(z\mid x)\| p(z)),
$$
...
$$
q(z\mid x) = \int q(z\mid \tilde{x})q(\tilde{x}\mid x)\mathrm{d}\tilde{x}.
$$
...
When discussing this with Rajesh Ranganath, he points out a simple counterexample where $$\mathcal{L}_{\text{dvae}}>\log p(x)$$: Consider a one-dimensional latent variable model, with $$p(x,z)=\mathrm{Bernoulli}(z;0.5)$$. Let

$$
q(\tilde{x}\mid x) = \mathrm{Bernoulli}(\tilde{x}; 0.5),~
